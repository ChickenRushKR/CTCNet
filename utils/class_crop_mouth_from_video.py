...
mean_face = './utils/20words_mean_face.npy'
crop_width = 96
crop_height = 96
start_idx = 48
stop_idx = 68
...
# -- mean face utils
STD_SIZE = (256, 256)
mean_face_landmarks = np.load(mean_face)
...
# parser.add_argument('--filename-path', default='./lrw500_detected_face.csv', help='list of detected video and its subject ID')
# parser.add_argument('--save-direc', default=None, help='the directory of saving mouth ROIs')
def crop_mouth(video_direc=None, landmark_direc=None, filename_path='./lrw500_detected_face.csv', save_direc=None):
    ...
    for filename_idx, line in enumerate(lines):
        ...
        video_pathname = os.path.join(video_direc)
        landmarks_pathname = os.path.join(landmark_direc)
        dst_pathname = os.path.join( save_direc, filename+'.npz')
